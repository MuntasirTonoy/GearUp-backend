...
const initiatePayment = async (customerId: string, rentalId: string) => {
  ...
  if (rental.customerId !== customerId) {
    const error: any = new Error('You are not authorized to pay for this rental');
    error.statusCode = httpStatus.FORBIDDEN;
    throw error;
  }

  if (rental.status !== 'APPROVED') {
    const error: any = new Error('Rental is not approved yet. Cannot initiate payment.');
    ...
    line_items: [
      {
        price_data: {
          ...
          unit_amount: Math.round(rental.totalAmount * 100), // Stripe expects amounts in cents
        },
        quantity: 1,
      },
    ],
...
const handleStripeWebhook = async (signature: string, rawBody: Buffer, type: 'success' | 'fail') => {
  let event;
  try {
  ...
  } else if (type === 'fail' && (event.type === 'checkout.session.async_payment_failed' || event.type === 'checkout.session.expired')) {
    const session: any = event.data.object;
    const rentalId = session.client_reference_id;
...
const getPaymentByRentalId = async (rentalId: string, userId: string, userRole: string) => {
